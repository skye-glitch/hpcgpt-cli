...
This directory contains the NCSA deployment of hpcGPT. It integrates Model Context Protocol (MCP) servers for Slurm-based HPC environments, Illinois Chat documentation Q&A, reporting, and Atlassian.
...
MCP servers in `NCSA/mcp_servers/*` use Bun/Node. From each server directory:
...
Opencode will also launch them automatically from the `NCSA/opencode.jsonc` `mcp` section when enabled.

## Environment Configuration

Use `NCSA/example.env` and `NCSA/example.env.atlassian` as references. Export directly or create `NCSA/.env` and `NCSA/.env.atlassian`.
...
Configure `NCSA/.env.atlassian` (see `NCSA/example.env.atlassian`). Common options:
